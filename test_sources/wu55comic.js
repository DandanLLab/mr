 ...
 * 对二进制图片字节有 UTF-8 解码损失。
 * 必须让软件用 ResponseType.bytes 通道下载 b_0 和 b_1。
 *
 * content() 对每个原图输出单个 <img src="b_1_url" data-parts='["b_0_url"]'>
 * - src 是主图 b_1 URL，软件用 ResponseType.bytes 下载
 * - data-parts 是 JSON 数组，存放所有配对切片 URL（支持任意数量）
 * - Dart 侧 _extractImageUrls 解析 data-parts 自动提取切片 URL
 * - DecodedImageProvider 下载主图后，额外下载所有切片
 * - 切片字节存入 globalThis._partBytes_N，数量存入 globalThis._partCount
 * - decryptImage 读取所有切片，AES 解密后合并，返回最终图
 *
 * 封面图（cover_pc.jpg）也是加密的，用 b_1 URL 作为 coverUrl。
 * 封面解密时 Dart 侧也会下载 b_0 并通过 data-parts 传入。
 *
 * 参考逆向:
...
// 单切片解密流程（只有 b_1，没有配对 b_0，如封面图）
function _fullDecryptSingle(d1) {
    if (!d1 || d1.length === 0) return null;

    // 解析 magic number
    var magic = parseMagicNumber(d1);
    var fileType = magic.fileType;

    // 注入图片头
    d1 = injectMagicHeader(d1, fileType);
    ...
    if (magic.bookId !== undefined && magic.pageNumber !== undefined &&
        ...
        if (stripCount > 0 && stripCount <= 80) {
            if (typeof __nativeImage !== 'undefined' && __nativeImage.scrambleRestore) {
                var restored = __nativeImage.scrambleRestore(d1, stripCount);
                if (restored) return restored;
            }
        }
    }
 ...
 *
 * result: 当前图片的原始字节（主图 b_1，软件用 ResponseType.bytes 下载）
 * src:    当前图片的 URL
 *
 * 单 img 方案（Dart 侧下载配对切片存入 JS 全局变量）：
 *   - Dart 侧下载主图后，额外下载 data-parts 指定的所有切片
 *   - 切片字节存入 globalThis._partBytes_0, _partBytes_1, ...
 *   - 切片数量存入 globalThis._partCount
 *   - decryptImage 读取所有切片，AES 解密后合并，返回最终图
 */
function decryptImage(result) {
    ...
    // AES 解密主图（b_1）
    var b1Decrypted = decryptSlice(result);
    if (!b1Decrypted || b1Decrypted.length === 0) {
        return null;
    }

    // 从 globalThis 读取配对切片
    var partCount = (typeof globalThis._partCount !== 'undefined') ? globalThis._partCount : 0;

    if (partCount === 0) {
        // 没有配对切片（如封面图），直接对 b_1 做条带重排后返回
        return _fullDecryptSingle(b1Decrypted);
    }

    // AES 解密所有配对切片
    var partsDecrypted = [];
    for (var i = 0; i < partCount; i++) {
        var partVar = '_partBytes_' + i;
        if (typeof globalThis[partVar] !== 'undefined') {
            var decrypted = decryptSlice(globalThis[partVar]);
            if (decrypted && decrypted.length > 0) {
                partsDecrypted.push(decrypted);
            }
        }
    }

    if (partsDecrypted.length === 0) {
        // 配对切片全部下载失败，降级用 b_1 单独处理
        return _fullDecryptSingle(b1Decrypted);
    }

    // 合并解密（b_0 + b_1），返回最终图
    return _fullDecrypt(partsDecrypted[0], b1Decrypted);
}
...
function content(result) {
    ...
    // 将 .jpg URL 转换为 b_0/b_1 URL，输出单 img + data-parts 属性
    // data-parts 存所有配对切片 URL（JSON 数组），支持任意数量切片
    // Dart 侧读取 data-parts 下载所有切片，合并后解密
    var imgTags = '';
    for (var j = 0; j < dataSrcs.length; j++) {
        ...
        // 单 img：src 是 b_1 URL（主图），data-parts 是配对切片 URL 数组
        imgTags += '<img src="' + b1Url + '" data-parts=\'["' + b0Url + '"]\'>\n';
    }
